(function(jQuery) {
  ...
  var triggerHtmlUpdate = function(event) {
    if (typeof htmlUpdateTimeoutId === 'number') {
      window.clearTimeout(htmlUpdateTimeoutId);
    ...
    htmlUpdateTimeoutId = window.setTimeout(function(event) {
  ...
  var updateHtmlFromMd = function(event) {
    ...
    var otherMdComment = event.data.otherMdComment;
    var htmlComment = event.data.htmlComment;
    var otherHtmlComment = event.data.otherHtmlComment;

    var mdText = mdComment.val();
    var otherMdText = otherMdComment === null ? null : otherMdComment.val();

    var htmlResult = md4mefi.md2html(mdText, otherMdText);
    htmlComment.val(htmlResult.htmlA);
    if (otherHtmlComment !== null) {
      otherHtmlComment.val(htmlResult.htmlB);
    }
    ...
    if (otherHtmlComment !== null) {
      triggerFakeEvent('keyup', otherHtmlComment.get(0));
    }
  };
  ...
  var turnOnLivePreview = function(event) {
    ...
    if (event.data.otherHtmlComment !== null) {
      triggerFakeEvent('focus', event.data.otherHtmlComment.get(0));
    }
  };
  ...
  var turnOffLivePreview = function(event) {
    ...
    if (event.data.otherHtmlComment !== null) {
      triggerFakeEvent('blur', event.data.otherHtmlComment.get(0));
    }
  };
  ...
  var doLink = function(event) {
    ...
    var selection = mdComment.selection();
    var mdText = mdComment.val();
    var otherMdText = event.data.otherMdComment !== null ? 
      event.data.otherMdComment.val() : null;
    var nextLinkNumber = md4mefi.nextLinkNumber(mdText, otherMdText);
    var linkText, linkUrl, caretPos, newlines;
    ...
    newlines = md4mefi.mdEndsInLinkReference(mdText) ? "\n" : "\n\n";
  ...
  var setupMarkdown = function(htmlComment, otherHtmlComment, mdComment, otherMdComment, includeToolbar) { 
  ...
  $(document).ready(function() {
    ...
    if (htmlComment.length === 1 && htmlExtended.length === 0) {
      ...
      mdComment = $('<textarea>');
      mdExtended = null;
      setupMarkdown(htmlComment, null, mdComment, mdExtended, true);
    }
    else if (htmlComment.length === 1 && htmlExtended.length === 1) {
      // Both comment & extended
      mdComment = $('<textarea>');
      mdExtended = $('<textarea>');
      setupMarkdown(htmlComment, htmlExtended, mdComment, mdExtended, true);
