(function(jQuery) {
  ...
  var triggerHtmlUpdate = function(event) {
    ...
    timeoutId = window.setTimeout(updateHtmlFromMd, 250, event);
  };
  ...
  var turnOnLivePreview = function(event) {
    ...
    if (htmlCommentDomNode !== undefined) {
  ...
  var addRevertToHtmlMessage = function(htmlComment, text) {    
    var messageArea = $('<div>')
  ...
  $(document).ready(function() {
    ...
    do {
      currSelector = postButtonSelectors.pop();
      postButton = form.find(currSelector);
    } while (postButton.length === 0 && postButtonSelectors.length > 0);
    ...
    do {
      currSelector = previewButtonSelectors.pop();
      previewButton = form.find(currSelector);
    } while (previewButton.length === 0 && previewButtonSelectors.length > 0);
