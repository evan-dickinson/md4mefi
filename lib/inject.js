(function(jQuery) {
  ...
  var $ = jQuery;
  var injectUtils = window.md4mefiInjectUtils;
  ...
  // Cached jquery selector lookup
  var jqueryLookup = (function() {
    var cache = {};
  ...
  var updateHtmlFromMd = function() {
    var mdComment  = jqueryLookup('.md4mefi-comment');
    var mdExtended = jqueryLookup('.md4mefi-extended');
  ...
  var receiveUpdatedHtml = function(result) {
    ...
    var htmlComment = jqueryLookup('#comment');
    var htmlExtended = jqueryLookup('#extended');
  ...
  var receiveNextLinkNumber = function(result) {
    ...
    var mdComment = isExtended ? jqueryLookup('.md4mefi-extended') : jqueryLookup('.md4mefi-comment');
  ...
  var addRevertToHtmlMessage = function(htmlComment, isError, text) {    
    ...
    var modernThemeToolbar = $(document).find('.html-shortcuts-toolbar');
    if (modernThemeToolbar.length > 0) {
      if (modernThemeToolbar.hasClass('postbar')) {
        // This class is what moves the toolbar after the textarea. Insert
        // before.
      ...
      // Clasic theme toolbar. Toolbar is always after the textarea,
      // so insert the message before the textarea.
      messageArea.insertBefore(htmlComment);
    }
  };
  ...
  $(document).ready(function() {
    ...
    // HTML mode.
    var staleness = saveRestore.isRestoredMarkdownStale({
      htmlCommentText: htmlComment.val(),
      htmlExtendedText: htmlExtended.val(),
      storedJson: storedJson,
      draftId: form.find('input[name="draft_id"]').val(),
    });

    if (staleness.isStale) {
      if (staleness.reason == 'draft') {
        addRevertToHtmlMessage(htmlComment, false, // not an error
          "Sorry, Markdown doesn't work when continuing a saved draft. " +
          "Switching back to HTML (classic MeFi editing)."
          );
      }
      else {
