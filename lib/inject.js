(function(jQuery) {
  ...
  var isPostPreviewPage = function(url) {
  ...
  var isCommentPreviewPage = function(url) {
  ...
  var getSessionStorageKey = function(linkId, formSubmitUrl) {
  ...
  var saveMarkdownForPreview = function(event) {
    ...
    var storageKey = getSessionStorageKey(event.data.linkId,
                                          event.data.formSubmitUrl);
  ...
  var restoreMarkdown = function(linkId, formSubmitUrl) {
    ...
    var isAllowedPath = isPostPreviewPage(window.location.pathname) ||
        isCommentPreviewPage(window.location.pathname);
    if (!isAllowedPath) {
      return null;
    }

    var storageKey = getSessionStorageKey(linkId, formSubmitUrl);
    if (storageKey === null) {
      return null;
    }
  ...
  $(document).ready(function() {
    ...
    var lnkId = (linkIdInput.length > 0) ? linkIdInput.val() : null;
    var formSubmitUrl = form.attr('action');

    var storedJson = restoreMarkdown(linkId, formSubmitUrl);
    if (storedJson !== null) {
    ...
    if (previewButton.length === 0) {
      ...
      // - "Step 2: Preview your thread"
      //
      // Match anything with "step 2" in the name.
      previewButton = form.find('input[type="submit"][value^="Step 2:"]');
    }
    // Second time visiting a new post page
    if (previewButton.length === 0) {
